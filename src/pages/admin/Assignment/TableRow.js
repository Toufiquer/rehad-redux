...
import { useDeleteAssignmentMutation } from "../../../redux/features/assignment/assignmentApi";
import { toast } from "react-toastify";

const TableRow = ({ data }) => {
  const [toggle, setToggle] = useState(false);
  const { video_title, title, totalMark, video_id, id } = data || {};
  console.log(video_id, id, " => Line No: 9");
  ...
  return (
    <>
      <tr>
        <td title={title} className="table-td">
        ...
        <td className="table-td flex gap-x-2">
          ...
          {/* Edit */}
          <Link to={`/admin/assignments/update/${id}`}>
            <svg
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth="1.5"
