...
const UpdateAssignment = () => {
  ...
  const [
    updateAssignment,
    { isSuccess, isLoading, isError, error: updateAssignmentError },
  ] = useUpdateAssignmentMutation();
  useEffect(() => {
  ...
  useEffect(() => {
    if (isError) {
      setError(updateAssignmentError.error);
    }
  }, [isError, updateAssignmentError]);
  useEffect(() => {
    if (isSuccess) {
      clearForm();
  ...
  useEffect(() => {
    const { totalMark, title, video_id, video_title } = loadData || {};
    if (title) {
      setTitle(title);
  ...
  const handleSubmit = (e) => {
    ...
    updateAssignment({ id, data: { ...currAssignment } });
  };
